...
with tempfile.TemporaryDirectory() as tmp:
    ...
    try:
        with sync_playwright() as p:
            ...
            # '/' is the public marketing page; owner sign-in lives at /login
            page.goto(base+'/login');page.locator('#password').fill('test-owner-password');page.get_by_role('button',name='Enter workspace').click();expect(page).to_have_url(base+'/workspace');expect(page.locator('#stat-total')).to_have_text('2')
            ...
            page.set_viewport_size({'width':390,'height':844});assert page.evaluate('document.documentElement.scrollWidth<=innerWidth');page.screenshot(path=str(Path(tmp)/'project-board-mobile.png'),full_page=True)
            page.evaluate("navigate('settings')");page.get_by_role('button',name='Sign out of owner session').click();expect(page).to_have_url(base+'/login');assert page.request.get(base+'/api/projects').status==401
            assert not errors,errors;print('PASS: owner login/CSRF, private pages, lead filters, evidence review, PDF downloads, project board, overdue reminders, mobile, logout. Disposable records only.');b.close()
