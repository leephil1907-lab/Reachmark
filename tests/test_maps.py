...
from tests.test_app import ProspectTests, module
from web.maps import grid, LIMIT
import web.map_provider as map_provider
class MapTests(unittest.TestCase):
    ...
    def start_scan(self, bounds=None):
        with patch('web.maps.threading.Thread'):
    ...
    def test_cell_resume_preserves_success_and_deduplicates(self):
        ...
        with patch('web.maps.query_overpass',side_effect=[payload,ValueError('offline'),payload,payload]), self.assertLogs(module.app.logger,level='WARNING'): worker(sid)
        ...
        with patch('web.maps.threading.Thread'): self.assertEqual(self.client.post(f'/api/map/scans/{sid}/resume',json={}).status_code,202)
        with patch('web.maps.query_overpass',return_value=payload) as query: worker(sid); self.assertEqual(query.call_count,1)
        self.assertEqual(self.client.get('/api/map/state').json['scans'][0]['state'],'checked')
        self.assertEqual(len(self.client.get('/api/state').json['leads']),1)
    def test_partial_never_checked(self):
        sid=self.start_scan()
        with patch('web.maps.query_overpass',return_value={'elements':[], 'remark':'runtime timeout'}): module.app.extensions['map_worker'](sid)
        scan=self.client.get('/api/map/state').json['scans'][0]
    ...
    def test_cap_is_partial_and_existing_lead_survives(self):
        ...
        with patch('web.maps.query_overpass',return_value={'elements':[element]*(LIMIT+1)}): module.app.extensions['map_worker'](sid)
        scan=self.client.get('/api/map/state').json['scans'][0]
        self.assertEqual(scan['cells'][0]['state'],'partial')
    ...
    def test_restart_recovers_queued_scan(self):
        ...
        result=subprocess.run([sys.executable,'-c','import web.app'],env={**os.environ,'DATABASE_PATH':module.DB},capture_output=True,timeout=20)
    ...
    def test_cancel_stops_unstarted_cells(self):
        ...
        with patch('web.maps.query_overpass') as query: module.app.extensions['map_worker'](sid);query.assert_not_called()
    ...
    def test_analytics_includes_actual_map_tasks(self):
        ...
        with patch('web.maps.query_overpass',return_value={'elements':[]}): module.app.extensions['map_worker'](sid)
        data=self.client.get('/api/analytics').json
    ...
    def test_place_cache(self):
        with patch('web.maps.geocode',return_value={'lat':'6.5','lon':'3.3','display_name':'Test place'}) as geo:
    ...
    def test_provider_fallback_and_rate_limit(self):
        ...
        import requests
        with patch.object(map_provider,'ENDPOINTS',['https://a.test','https://b.test']),patch.object(map_provider,'_cooldown',{}),patch.object(map_provider,'_rate_until',0),patch('web.map_provider.time.sleep'),patch('web.map_provider.requests.post',side_effect=[requests.Timeout(),response]) as post:
