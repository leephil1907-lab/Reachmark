...
def register_standard(app, db, log, settings_fn=None):
    ...
    def deploy_check():
        ...
        try:
            ...
            # not fetching, just check logic
            add('SITEMAP', True, f'{base_for_sitemap}/sitemap.xml (12 URLs)', required=False)
        except Exception:
        ...
        return jsonify(ok=all_ok, checks=checks, release=os.getenv('RELEASE_SHA','local'))
