...
Feature modules may create their own feature tables, but all core tables and
core column migrations live here so startup has one authoritative schema path.
"""
...
def initialize_database(db):
    with db() as c:
        ...
        # Migration 1: normalize legacy core columns. Safe on both fresh and old DBs.
        applied = {row[0] for row in c.execute("SELECT version FROM schema_migrations")}
        if 1 not in applied:
            _add_missing(c, "leads", {k: v for k, v in LEAD_COLUMNS.items() if k != "source_seen_at"})
            _add_missing(c, "jobs", {"owner_user_id": "TEXT"})
            c.execute(
                "INSERT INTO schema_migrations(version, applied_at) "
                "VALUES(1, datetime('now'))"
            )

        # Migration 2: source_seen_at is the canonical timestamp for source refreshes.
        if 2 not in applied:
            _add_missing(c, "leads", {"source_seen_at": "TEXT"})
            c.execute(
                "INSERT INTO schema_migrations(version, applied_at) "
                "VALUES(2, datetime('now'))"
            )

        # Existing installations can have the migration ledger but still be missing
        # a column because an older startup was interrupted. Always self-heal columns.
        _add_missing(c, "leads", LEAD_COLUMNS)
        _add_missing(c, "jobs", {"owner_user_id": "TEXT"})

        # Queued/running work cannot survive a process restart safely.
        c.execute(
