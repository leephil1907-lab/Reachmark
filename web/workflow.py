...
from web.operations import CURRENCIES,money
...
def register_workflow(app,db,now,log):
    ...
    def save_review(lid):
        ...
        if not isinstance(status,str) or status not in VERIFICATIONS or not isinstance(note,str) or not isinstance(url,str) or len(note)>3000 or len(url)>1000:return jsonify(error=_t('er_093', locale_now())),400
        try:
            parsed=urlsplit(url)
        ...
        if not validurl:return jsonify(error=_t('er_049', locale_now())),400
        if status not in ('UNREVIEWED','INCONCLUSIVE') and (len(note.strip())<20 or not url):return jsonify(error=_t('er_007', locale_now())),400
        with db() as c:
    ...
    def project_write(pid=None):
        ...
        if request.method in ('POST','PATCH','DELETE') and session.get('client_id') and not session.get('owner'):
            return jsonify(error=_t('er_082', locale_now())),403
        ...
        if any(not isinstance(v.get(k,''),str) for k in fields if k in v):return jsonify(error=_t('er_090', locale_now())),400
        ...
        if not title or len(title)>180 or stage not in STAGES or currency not in CURRENCIES or len(v.get('scope',''))>10000 or len(v.get('next_action',''))>500:return jsonify(error=_t('er_095', locale_now())),400
        ...
        if client_user_id:
            with db() as c:
                if not c.execute('SELECT 1 FROM users WHERE id=? AND role=\"client\"',(client_user_id,)).fetchone():
                    return jsonify(error=_t('er_023', locale_now())),400
        try:
        ...
        except ValueError:return jsonify(error=_t('er_046', locale_now())),400
        ...
        with db() as c:
            ...
            if lead and (not _l or (_cid and _l['owner_user_id'] != _cid)):return jsonify(error=_t('er_063', locale_now())),400
            if _cid and contract:return jsonify(error=_t('er_064', locale_now())),400
            if contract and not c.execute('SELECT 1 FROM contracts WHERE id=?',(contract,)).fetchone():return jsonify(error=_t('er_064', locale_now())),400
