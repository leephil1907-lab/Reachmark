...
function render(){
 ...
 $('#recent-leads').innerHTML=state.leads.length?state.leads.slice(0,4).map(l=>recentRow(l)).join(''):empty('Your next opportunity is out there.','Discover local businesses or import your first lead list.','<button class="text-link" data-rm-click="openDiscover">Find businesses ↗</button>');
...
function recentRow(l,tab='details'){return `<div class="recent-row" role="button" tabindex="0" data-rm-click="openLead" data-rm-json='${JSON.stringify([l.id, tab])}' data-rm-keydown="openLead" data-rm-key="Enter" data-rm-json='${JSON.stringify([l.id, tab])}'><span class="business-icon">${esc(l.name.slice(0,1).toUpperCase())}</span><div><div class="business-name">${esc(l.name)}</div><div class="business-sub">${esc(l.category||'Local business')} · ${esc(l.city||'Location not added')}</div></div>${badge(l.status)}<span class="muted">↗</span></div>`}
function navigate(page){if(typeof inboxDirty==='function'&&inboxDirty()&&page!=='enquiries'&&!confirm('Leave this page and discard unsaved enquiry changes?'))return;document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id==='page-'+page));document.querySelectorAll('.nav').forEach(p=>p.classList.toggle('active',p.dataset.page===page));$('#breadcrumb').textContent={overview:'Overview',leads:'Lead directory',outreach:'Outreach studio',settings:'Settings',global:'Global finder',health:'Website health',portfolio:'Website samples',enquiries:'Enquiry inbox',contracts:'Contracts',projects:'Projects & follow-ups',invoices:'Invoices'}[page];window.scrollTo({top:0,behavior:'smooth'});history.replaceState(null,'','#'+page);if(page==='projects'&&typeof loadProjects==='function')loadProjects();if(page==='global'&&typeof openWorldMap==='function')openWorldMap();if(page==='enquiries'){if(typeof loadEnquiries==='function')loadEnquiries();if(typeof loadClientReviews==='function')loadClientReviews();}if(page==='contracts'&&typeof loadContracts==='function')loadContracts();if(page==='overview'&&typeof loadAnalytics==='function')loadAnalytics()}
document.querySelectorAll('.nav').forEach(b=>b.onclick=()=>navigate(b.dataset.page));
function renderLeads(){if(typeof syncQualityFilters==='function')syncQualityFilters();let q=$('#search').value.toLowerCase(),status=$('#filter-status').value,stage=$('#filter-stage').value;let leads=state.leads.filter(l=>(typeof qualityMatches!=='function'||qualityMatches(l))&&(!q||[l.name,l.city,l.category,l.email].join(' ').toLowerCase().includes(q))&&(!status||l.status===status)&&(!stage||l.stage===stage));$('#lead-table').innerHTML=leads.length?leads.map(l=>`<tr><td><div class="business-cell"><span class="business-icon">${esc(l.name[0])}</span><div><div class="business-name">${esc(l.name)}</div><div class="business-sub">${esc(l.category||'Local business')}</div></div></div></td><td>${esc(l.city||'—')}</td><td>${badge(l.status)}</td><td><span class="badge ${['Replied','Won'].includes(l.stage)?'green':''}">${esc(l.stage)}</span></td><td>${l.email?'<span style="color:#718d81">Email available</span>':l.phone?'Phone available':'Needs research'}</td><td><button class="text-link" data-rm-click="openLead" data-rm-arg="${l.id}">Open ↗</button></td></tr>`).join(''):`<tr><td colspan="6">${empty(state.leads.length?'No matching businesses.':'A clean slate. A lot of potential.',state.leads.length?'Try a different search or filter.':'Discover businesses, add one manually, or import a CSV.')}</td></tr>`;$('#table-count').textContent=`${leads.length} of ${state.leads.length} businesses · Saved to your workspace`}
...
function csvImportClick(){
  var f=document.getElementById('csv-file');
  if(f)f.click();
}
function closeModalAndOpenLead(modal,id,tab){
  closeModal(modal);
  openLead(id,tab);
}
...
refresh().then(()=>{const page=location.hash.slice(1);if(['overview','leads','outreach','settings','global','health','portfolio','enquiries','contracts','projects'].includes(page))navigate(page)}).catch(e=>toast(e.message,true));
...
// --- Client review moderation (owner only; public submissions start unapproved) ---
async function loadClientReviews(){
  try{
    const me=await fetch('/api/auth/me').then(r=>r.json().catch(()=>({})));
    if((me.role||'none')!=='owner')return;
    const box=document.getElementById('review-moderation-list');
    if(!box)return;
    const rows=await api('/api/client-reviews');
    const pending=rows.filter(r=>!r.approved).length;
    const badge=document.getElementById('review-count');
    if(badge){badge.textContent=pending?pending+' pending':'All clear';badge.className='badge '+(pending?'amber':'green');}
    if(!rows.length){box.innerHTML=empty('No reviews yet.','Submissions from the public reviews wall will appear here before they can go live.');return;}
    box.innerHTML=rows.map(r=>{
      const stars='\u2605'.repeat(r.rating)+'\u2606'.repeat(5-r.rating);
      const status=r.approved?'<span class="badge green">Published</span>':'<span class="badge amber">Hidden</span>';
      return `<div class="review-moderation-row" style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;padding:14px 0;border-top:1px solid #e3e8da"><div><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><strong>${esc(r.name)}</strong><span class="muted small">${stars}</span>${status}</div>${r.business?`<div class="small muted">${esc(r.business)}</div>`:''}<p class="small" style="margin:8px 0 0;line-height:1.6">${esc(r.text)}</p><small class="muted">${new Date(r.created).toLocaleString()}</small></div><div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">${r.approved?`<button class="text-link" data-rm-click="moderateReview" data-rm-json='${JSON.stringify([r.id, "reject"])}'>Unpublish</button>`:`<button class="secondary" data-rm-click="moderateReview" data-rm-json='${JSON.stringify([r.id, "approve"])}'>Approve</button>`}<button class="text-link" style="color:#a33" data-rm-click="moderateReview" data-rm-json='${JSON.stringify([r.id, "delete"])}'>Delete</button></div></div>`;
    }).join('');
  }catch(e){}
}
async function moderateReview(rid,action){
  if(action==='delete'&&!confirm('Delete this review permanently?'))return;
  try{
    await api('/api/client-reviews/'+rid+'/moderate','POST',{action});
    toast(action==='approve'?'Review published.':'Review updated.');
...
async function applyRoleView(){
  try{
    ...
    const ownerOnly = ['global','leads','health','contracts'];
