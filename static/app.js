...
refresh().then(()=>{const page=location.hash.slice(1);if(['overview','crew','leads','outreach','settings','global','health','portfolio','enquiries','contracts','projects','invoices','clients'].includes(page))navigate(page)}).catch(e=>toast(e.message,true));
...
async function applyRoleView(){
  try{
    ...
    const ownerOnly = ['global','leads','health','contracts','crew','clients'];
