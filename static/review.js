...
(function () {
  ...
  var chosen = '';
  var LABELS = {
    want: 'Yes — build my website',
    later: 'Not right now',
    have: 'I already have a website'
  };
  var CONFIRM = {
    want: 'The studio owner has your answer and will come back with scope and a fixed price. Nothing is charged until you approve it.',
    later: 'Your answer is saved and nothing else will be sent. The concept stays on file in case it becomes a priority.',
    have: 'Understood — the studio will not pitch a rebuild. If you add a note about what you would improve, they will read it.'
  };
  ...
  choices.forEach(function (choice) {
    choice.addEventListener('click', function () {
      ...
      label.textContent = 'Your answer: ' + LABELS[chosen];
      form.hidden = false;
  ...
  form.addEventListener('submit', function (event) {
    ...
    if (!chosen) { label.textContent = 'Pick one of the three answers first.'; return; }
    var payload = {
      choice: chosen,
    ...
    fetch('/api/r/' + encodeURIComponent(token) + '/respond', {
      ...
      if (!result.ok) {
        ...
        button.textContent = 'Send my answer ↗';
        label.textContent = (result.data && result.data.error) || 'Something went wrong — please try again.';
        return;
      }
      form.hidden = true;
      done.hidden = false;
      doneCopy.textContent = CONFIRM[chosen] || 'Your answer has been sent.';
      done.scrollIntoView({ block: 'center', behavior: 'smooth' });
    }).catch(function () {
      button.disabled = false;
      button.textContent = 'Send my answer ↗';
      label.textContent = 'The connection dropped — please try again.';
    });
  });
