...
function drawWorldLeads(){
 ...
 rows.slice(0,5000).forEach(l=>{const content=document.createElement('div');content.className='map-popup';content.innerHTML=`<strong>${esc(l.name)}</strong><span>${esc(l.category)} · ${esc(l.city)}</span><br><span>${esc(statusLabel(l.status))}</span><br>`;const btn=document.createElement('button');btn.textContent=T_('wsj.mp_review','Review business ↗');btn.onclick=()=>openLead(l.id);content.appendChild(btn);L.marker([l.latitude,l.longitude],{title:l.name,icon:L.divIcon({className:'world-pin',html:`<span style="background:${l.status==='HAS_WEBSITE'?'#80abc0':'#cee77c'}"></span>`,iconSize:[16,16],iconAnchor:[8,8]})}).bindPopup(content).addTo(worldMarkers)});
 $('#map-records').textContent=T_('wsj.mp_records','{m} mapped{lim} · {w} without valid coordinates').replace('{m}',Math.min(rows.length,5000)).replace('{lim}',rows.length>5000?T_('wsj.mp_lim',' (5,000 display limit)'):'').replace('{w}',state.leads.length-located.length);
}
async function openWorldMap(){
 if(!window.L){mapMessage(T_('wsj.mp_nolib','Map library unavailable. Reload the page; your saved leads remain in the directory.'));return}
 if(!worldMap){
...
async function loadMapScans(){
 ...
 catch(e){mapMessage(T_('wsj.mp_stale','Could not refresh scan progress. Last displayed progress is retained. {e}').replace('{e}',e.message))}finally{worldRequest=false}
}
function drawMapScans(){
 if(!worldMap)return;worldCoverage.clearLayers();
 for(const scan of worldScans)for(const cell of scan.cells){const [s,w,n,e]=cell.bounds;const rect=L.rectangle([[s,w],[n,e]],{color:mapColors[cell.state]||'#838a94',weight:2,fillOpacity:.1});const text=document.createElement('div');text.className='map-popup';text.innerHTML=`<strong>${esc(scan.label)}</strong>${esc(scan.category)} · ${esc(cell.state)}<br>${T_('wsj.mp_hits','{n} listing hits').replace('{n}',cell.found)}<br>${esc(cell.message)}<br><small>${esc(new Date(cell.updated).toLocaleString())}</small>`;rect.bindPopup(text).addTo(worldCoverage)}
 worldMarkers.eachLayer(layer=>layer.bringToFront?.());
 $('#map-history').innerHTML=worldScans.length?worldScans.map(s=>{const checked=s.cells.filter(c=>c.state==='checked').length,retry=s.cells.some(c=>['pending','failed','running'].includes(c.state)),active=['queued','running'].includes(s.state);return `<article class="map-scan-row"><div><strong>${esc(s.label)}</strong><p class="small">${esc(s.category)} · ${esc(s.state)} · ${T_('wsj.mp_cells','{c}/{t} cells checked').replace('{c}',checked).replace('{t}',s.cells.length)}</p><p class="small muted">${esc(new Date(s.created).toLocaleString())} · ${T_('wsj.mp_hitso','{n} listing hits (may overlap)').replace('{n}',s.cells.reduce((n,c)=>n+c.found,0))}</p></div><div class="map-scan-actions"><button type="button" class="text-link" data-map-fit="${s.id}">${T_('wsj.mp_view','View area')}</button>${active?`<button type="button" class="text-link" data-map-action="cancel" data-id="${s.id}">${T_('wsj.mp_pause','Pause')}</button>`:retry?`<button type="button" class="text-link" data-map-action="resume" data-id="${s.id}">${T_('wsj.mp_resume','Resume unfinished')}</button>`:''}</div></article>`}).join(''):'<p class="small muted">'+T_('wsj.mp_none','No areas scanned yet. Navigate to a neighbourhood, choose a category and scan the visible area.')+'</p>';
 ...
 $('#map-history').querySelectorAll('[data-map-action]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await api(`/api/map/scans/${b.dataset.id}/${b.dataset.mapAction}`,'POST',{});await loadMapScans();mapMessage(b.dataset.mapAction==='cancel'?T_('wsj.mp_pauseq','Pause requested. An in-flight request may finish saving its cell.'):T_('wsj.mp_resumeq','Resuming unfinished cells; checked cells will not be repeated.'))}catch(e){mapMessage(e.message)}finally{b.disabled=false}});
}
